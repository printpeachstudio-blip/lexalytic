        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: '20px' }}>
          {services.map((s, i) => (
            <Link key={i} href={`/services/${s.slug}`} style={{
              ...
              <h3 style={{ marginBottom: '12px', fontSize: '1.05rem', fontFamily: 'var(--serif)', color: s.highlight ? 'var(--white)' : 'var(--ink)', letterSpacing: '-0.01em' }}>{s.title}</h3>
              <p style={{ fontSize: '14px', lineHeight: '1.75', marginBottom: '24px', color: s.highlight ? 'rgba(255,255,255,0.55)' : 'var(--ink-3)' }}>{s.desc}</p>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '20px' }}>
                {s.tags.map(t => (
                  <span key={t} style={{ fontSize: '11px', fontWeight: '500', color: s.highlight ? 'rgba(255,255,255,0.45)' : 'var(--ink-3)', background: s.highlight ? 'rgba(255,255,255,0.08)' : 'var(--bg-2)', padding: '4px 10px', borderRadius: '100px', border: s.highlight ? 'none' : '1px solid var(--border)' }}>{t}</span>
                ))}
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${s.highlight ? 'rgba(255,255,255,0.08)' : 'var(--border)'}`, paddingTop: '16px' }}>
                <span style={{ fontSize: '12px', color: s.highlight ? 'rgba(255,255,255,0.3)' : 'var(--ink-4)' }}>{s.time}</span>
